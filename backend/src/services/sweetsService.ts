 ...
 * SEARCH SWEETS
 */
export async function searchSweets(filters: any) {
  const where: any = {};
 ...
 */
export async function purchaseSweet(id: number, qty: number) {
 ...
 * RESTOCK SWEET
 */
export async function restockSweet(id: number, qty: number) {
  return prisma.sweet.update({
    where: { id },
    data: {
